...
autoplot(solar_decompose_a, main = "Solar Decomposition - Additive")
#the smaller the bar, the more significant the component is. ie don't have to zoom in as much

wind_decompose_a <- decompose(ts_wind, "additive")
autoplot(wind_decompose_a, main = "Wind Decomposition - Additive")
```
...
#close to zero at the beginning of the data accounts for the high variation in random series at the beginning
autoplot(solar_decompose_m, main = "Solar decomposition - multiplicative")

wind_decompose_m<- decompose(ts_wind, "multiplicative")
autoplot(wind_decompose_m, main = "Wind decomp - multiplicative")
```
...
autoplot(solar_decomp_recent, main = "recent solar decomp-additive")

wind_decomp_recent <- decompose (ts_wind_recent, "additive")
autoplot(wind_decomp_recent, main = "rencent wind decomp-additive")
```
...
Both series removed the seasonal component, particularly in the years since 2015. The cleaned lines are far more linear than the original lines which oscillate up and down significantly. 
