...
```{r}
ts_inflow_data <- ts(inflow_data[,2:4], #don't want to include first column because data 
                     start=c(1931,1), #need to specify the first observation (because this is monthly data only need to specify year and month)
                     frequency=12) #monthly data so frequency is 12. daily would be 365. annual would be 1. We will talk about how to do hourly data. ts only works with only one frequency variable
ts_inflow_data
...
autoplot(ts_inflow_data) + #don't need to specify aesthetics because it knows its a timeseries object. Could do this with ggplot but its much more set up with autoplot
  xlab("Time") +
  ylab("Inflow") +
...
HP1_acf=acf(ts_inflow_data[,1],lag.max = 40, plot = TRUE) #default type is correlation. can specify partial here or use pacf
#shows lags between 0 and 3 (though 40 lines so the lags are not labeled as they should). Another reason to use Acf() instead
...
#The PACF eventually levels out between the blue lines, with a few spikes due to the seasonality
HP1_acf= Acf(ts_inflow_data[,1], lag.max = 40) #doesn't include 0, always real number lags

HP1_pacf=Pacf(ts_inflow_data[,1], lag.max = 40)
