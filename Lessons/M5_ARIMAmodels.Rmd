...
#could also subtract seasonal from original manually
...
plot_grid(
  autoplot(Pacf(deseasonal_residential_price, lag = 40, plot=FALSE), 
                main = "Deseason Residential Electricity Price"),
  autoplot(Pacf(electricity_price_processed$Residential, lag = 40, plot=FALSE),  
                  main = "Original Residential Electricity Price")
)
...
#large p value, accept null hypothesis that my time series has a unit root (is stochastic). Needs to be differenced
...
#in ARIMA, d = 1 because we differenced once. Can provide the deseas/differenced series or can just provide the deseasonal data and specify d = 1 and ARIMA will do for you
```
...
#we don't see any clear cut offs so we assume this will be ARMA
Model_111 <- Arima(deseasonal_residential_price,
                   order=c(1,1,1), include.drift = TRUE)
print(Model_111)

#see spikes but not patterns in ACF
...
Model_011 <- Arima(deseasonal_residential_price,
                   order=c(0,1,1), include.drift = TRUE)
